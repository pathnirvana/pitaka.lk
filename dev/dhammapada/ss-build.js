...
const lines = fs.readFileSync(__dirname + '/gatha2.txt', { encoding: 'utf8' });
let vaggas = lines.split('@'); console.log(`num of vaggas ${vaggas.length}`); //vaggas = vaggas.slice(0, 6); //ONLY TAKE THE FIRST

let kathaIndex = 0, gathaChecker = 89, vaggaH1, kathaH2, allFootnotes;
...
(async () => {
    ...
    for(const wordFile of wordDocList) {
        htmlStr += (await getHtmlStr(wordFile))
    }
    ...
    console.log(`num vaggas = ${vaggaH1.length}, kathas = ${kathaH2.length} in html`);
...
function processVagga(vagga, vaggaInd) {
    ...
    return JC('a', 'vagga-link').attr('id', `vagga-${vaggaInd}`).attr('href', getVaggaFileName(vaggaInd))
...
function writeKathaFile(gathas, vaggaInd, vaggaName) {
    const [kathaTitle, kathaHead] = getKathaHeading($(kathaH2[kathaIndex++]));
...
function getKathaHeading(kathaHead) {
    assert(kathaHead.text().trim(), `katha heading '${kathaHead.text()}' does not follow the standard`);
    const numText = kathaHead.next().text().trim()
    assert(/^(\d+)\s*[-–]\s*(\d+)/.exec(numText) && kathaHead.next().is('.katha-ending'),
        `katha number '${numText}' of '${kathaHead.text()}' does not follow the standard`)
